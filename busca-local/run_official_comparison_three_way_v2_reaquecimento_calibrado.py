"""Comparação de três vias — v2, cooling_rate v2 + reaquecimento RECALIBRADO.

Rodada PARALELA às comparações anteriores (run_official_comparison_three_way.py
e run_official_comparison_three_way_v2_schedule_corrigido.py), não um
substituto: mesmas seeds (seed_base=2026), mesmo n_runs=30, mesmo
total_evaluations=20000, mesmo HC (hill_climbing_fixed_budget, inalterado) e
mesmo cooling_rate v2 (calculado por `busca_local.sa_utils.compute_cooling_rate`)
para SA e SA-reaquecido.

A diferença desta rodada: o SA-reaquecido usa os NOVOS defaults de
`simulated_annealing_reheating.py` — `stagnation_limit=1000` (era 500) e
`reheat_factor=1.3` (era 3.0). Motivo: na rodada anterior
(comparacao_tres_vias_v2_schedule_corrigido.csv), reheat_factor=3.0 aplicado
sobre o cooling_rate v2 (que mantém T alta por muito mais tempo) triplicava
uma temperatura já alta, tornando a busca essencialmente aleatória —
resultado: só 1/30 execuções viáveis, custo médio mais que dobrado frente ao
SA puro. Esta rodada testa se um reaquecimento mais conservador (empurrão de
30%, com menos frequência) recupera um comportamento útil.

Resultados salvos em resultados/comparacao_tres_vias_v2_reaquecimento_calibrado.csv
(os CSVs anteriores permanecem intocados).

Uso: python run_official_comparison_three_way_v2_reaquecimento_calibrado.py
"""
...
CSV_PATH = RESULTS_DIR / "comparacao_tres_vias_v2_reaquecimento_calibrado.csv"
...
# Reaquecimento recalibrado (ver docstring do modulo): empurrao mais leve,
# aplicado com menos frequencia, do que os defaults exploratorios originais.
...
ALGORITHMS = (
    ...
    "simulated_annealing_reheating_calibrado",
)
...
def main():
    ...
    print(f"reaquecimento recalibrado: stagnation_limit={REHEAT_STAGNATION_LIMIT}  "
          f"reheat_factor={REHEAT_FACTOR}  (eram 500 e 3.0)")
    print()
    ...
    print(f"=== Comparacao de tres vias v2 (schedule corrigido + reaquecimento calibrado) — n_runs={N_RUNS}, "
    ...
    pairs = [
        ...
        ("hill_climbing_fixed_budget", "simulated_annealing_reheating_calibrado"),
        ("simulated_annealing_v2", "simulated_annealing_reheating_calibrado"),
    ]
    ...
    print(f"  SA-reaquecido-calibrado: total accepted_worsening_moves_after_feasibility = {sa_reheating_worsening_total}")
    print(f"  SA-reaquecido-calibrado: total de eventos de reaquecimento (soma das 30 execucoes) = {reheating_events_total}")
    print()
